...
# 1.1 直接效果模型 (X → Y)
model_total = lm(主動攻擊分數 ~ 被動攻擊分數, data = mediation_data)
cat("【1.1】總效果模型 (X → Y)\n")
...
# 1.2 X → M (第一階段)
model_xm = lm(道德正當化指數 ~ 被動攻擊分數, data = mediation_data)
cat("【1.2】第一階段 (X → M)\n")
...
# 1.3 X + M → Y (第二階段)
model_xmy = lm(主動攻擊分數 ~ 被動攻擊分數 + 道德正當化指數, data = mediation_data)
cat("【1.3】第二階段 (X, M → Y)\n")
...
for(i in 1:n_boot) {
  ...
  m1 = lm(道德正當化指數 ~ 被動攻擊分數, data = boot_data)
  m2 = lm(主動攻擊分數 ~ 被動攻擊分數 + 道德正當化指數, data = boot_data)
...
cat("Nagelkerke R²:", round(lrm(高主動攻擊 ~ 年齡_標準化+性別_numeric+被動攻擊_標準化+
                                  道德正當化_標準化+快樂度_標準化, data = logistic_data)$stats['R2'], 3), "\n")
...
# 2.3 模型擬合度
cat("\n【2.3】模型擬合度評估\n")
AIC_full1 = AIC(model_logistic_full1)
BIC_full1 = BIC(model_logistic_full1)
cat("AIC:", round(AIC_full1, 2), "\n")
cat("BIC:", round(BIC_full1, 2), "\n")

# Nagelkerke R²
library(rms)
library(Hmisc)
cat("Nagelkerke R²:", round(lrm(低主動攻擊 ~ 年齡_標準化+性別_numeric+被動攻擊_標準化+
                                  道德正當化_標準化+快樂度_標準化, data = logistic_data1)$stats['R2'], 3), "\n")
...
# ROC 圖
library(ggplot2)
p_roc1 = ggroc(roc_obj1, color = "#E31A1C", size = 1.2) +
  geom_abline(intercept = 1, slope = 1, linetype = "dashed", color = "gray50") +
  annotate("text", x = 0.6, y = 0.2, 
           label = paste0("AUC = ", round(auc_value, 3)), 
           size = 5, fontface = "bold") +
...
# 2.5 OR 值圖表
p_or1 = ggplot(logistic_results1, aes(x = OR, y = reorder(變項, OR))) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "red", size = 1) +
  geom_point(color = "black", size = 3) +
  geom_errorbarh(
    aes(xmin = CI_下, xmax = CI_上),
...
library(dplyr)
anova_data = data %>%
  dplyr::select(主動攻擊分數, 被動攻擊分數, 年齡組, 性別, 出生地區) %>%
  na.omit()
...
sem_data = data %>%
  ...
  dplyr::select(
    被動攻擊, 主動攻擊, 道德正當化, 同理心, 滿意度, 年齡_c, 性別_dummy
  ) %>%
  na.omit()
...
cluster_data$同理心程度 = factor(cluster_data$同理心程度)
cluster_data$生活滿意度 = factor(cluster_data$生活滿意度)
cluster_data=scale(cluster_data,scale = T)
...
wss = sapply(1:10, function(k) {
  kp_temp = kproto(cluster_data, k, nstart = 20, verbose = 0)
  sum(kp_temp$withinss)
})

wss = c()
for(k in 1:10) {
  cat("計算 k =", k, "...\n")
  kp_temp = kproto(cluster_data, k, nstart = 10, verbose = 0)
...
cat("\n【5.2】K-Prototype 聚類 (k=3)\n")
set.seed(123)
kp = kproto(cluster_data, 3, nstart = 25, verbose = 0)
...
cat("群間 BSS：", round(kp$betweenss, 2), "\n")
...
# ====================================
cat("\n\n【6】調節效果分析：同理心調節被動攻擊→主動攻擊\n")

# 準備調節分析數據
moderation_data = data %>%
  mutate(
    被動攻擊_c = scale(被動攻擊分數)[,1],
    同理心_c = scale(同理心程度)[,1],
    互動項 = 被動攻擊_c * 同理心_c,
    主動攻擊 = 主動攻擊分數
  ) %>%
  dplyr::select(主動攻擊, 被動攻擊_c, 同理心_c, 互動項) %>%
  na.omit()
...
# 6.1 主模型（包含互動項）
cat("【6.1】調節迴歸模型\n")
model_mod = lm(主動攻擊 ~ 被動攻擊_c + 同理心_c + 互動項, data = moderation_data)
...
slope_low = coef(model_mod)["被動攻擊_c"] + coef(model_mod)["互動項"] * low_empathy
cat("低同理心（-1 SD）時的斜率：", round(slope_low, 4), "\n")

# 高同理心 (+1 SD)
high_empathy = mean(moderation_data$同理心_c) + sd(moderation_data$同理心_c)
slope_high = coef(model_mod)["被動攻擊_c"] + coef(model_mod)["互動項"] * high_empathy
cat("高同理心（+1 SD）時的斜率：", round(slope_high, 4), "\n\n")

library(tidyr)
# 6.3 調節效應可視化
plot_data = expand.grid(
  被動攻擊_c = seq(min(moderation_data$被動攻擊_c), 
               max(moderation_data$被動攻擊_c), length.out = 50),
...
p_moderation = ggplot(plot_data, aes(x = 被動攻擊_c, y = 預測值, 
